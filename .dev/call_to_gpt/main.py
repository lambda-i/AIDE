...
async def handle_incoming_call(
    ...
    caller_number = form_data.get("From", "Unknown")  # Extracts callers PH and callID
    # logger.info(f"Caller: {caller_number}")
    session_id = create_session(
        api_key, project_id, caller_number
    ...
    response = (
        VoiceResponse()
    )  # Creates the root TwiML response for controlling the call.
    response.pause(length=1)
    connect = (
        Connect()
    )  # Initializes a Connect TwiML instruction for real-time audio streaming.
    ...
    )  # Defines a WebSocket endpoint (url) for real-time audio streaming.
    stream.parameter(
        name="api_key", value=api_key
    )  # Adds a custom parameter (api_key) to the WebSocket connection.
    connect.append(stream)  # Attaches the stream to the Connect instruction.
    response.append(
        connect
    )  # Adds the Connect instruction (and its attached stream) to the TwiML response.
    if phone_number:
...
def get_additional_context(query, api_key, project_id, session_id):
    ...
    while tries <= max_retries:
        try:
            ...
            return (
                conversation.parsed.data.openai_response
            )  # Correct f-string is unnecessary
        except Exception as e:
            logger.error(
